...
📱 **Android** → 💻 **Windows / macOS**　｜　局域网通信，数据不经过任何服务器
...
Android 端采用浅绿色主题，界面简洁清爽。支持语音输入、图片传输，通知栏常驻快捷方式，方便随时打开使用。

<div align="center">
  <img src="docs/images/android-mobile.png" alt="VoiceSync Android 端界面" width="250"/>
  <p><i>Android 端界面：支持语音输入、图片选择、自动同步，通知栏快捷启动</i></p>
</div>
...
- ✅ **图片传输**：支持单张或多张选择手机相册图片并同步到电脑剪贴板，直接粘贴使用
- ✅ **通知栏快捷方式**：常驻通知栏，下拉状态栏即可快速打开应用
- ✅ **设备自动发现**：通过 mDNS/Bonjour 自动扫描局域网内的 Mac 设备，无需手动输入 IP
- ✅ **IP 地址历史**：自动记录所有连接过的电脑 IP，快速切换设备
- ✅ **自动更新检查**：自动检测新版本并提供下载
- ✅ **手动发送/清除**：保留手动控制按钮，随时干预

### 电脑端（接收端，支持 Mac / Windows）

- ✅ **HTTP 监听服务**：后台运行，实时接收手机发来的文字和图片
- ✅ **mDNS 服务广播**：自动向局域网广播服务，让 Android 设备能够自动发现
- ✅ **剪贴板自动注入**：收到文字或图片后立即更新系统剪贴板，直接粘贴
- ✅ **自动粘贴功能**：可选自动模拟粘贴快捷键（Mac: `Cmd+V` / Windows: `Ctrl+V`），内容直接出现在当前光标位置
- ✅ **响应远程回车**：可选是否执行手机端发来的回车指令
- ✅ **同步历史记录**：界面展示所有同步记录（包括图片），防止被后续剪贴板操作覆盖
- ✅ **托盘/菜单栏模式**：隐藏主窗口，常驻系统托盘（Windows）或菜单栏（Mac）运行
...
1. 从 [Releases](https://github.com/sedationh/VoiceSync/releases) 下载 `VoiceSync-Windows-x.x.x.zip`
2. 解压后双击 `VoiceSync.exe` 即可运行
3. 首次运行如弹出 Windows Defender 提示，点击「仍要运行」；防火墙提示请允许网络访问
4. 应用会自动在 `4500` 端口监听，并通过 mDNS 广播到局域网
5. 关闭窗口会最小化到系统托盘，右键托盘图标可退出
...
2. 确保手机和电脑连接在**同一个 Wi-Fi 网络**
3. 打开应用后，点击 IP 输入框旁边的**扫描按钮**（📡 图标）
4. 等待 1-3 秒，从列表中选择你的 Mac 设备
5. IP 地址会自动填充，无需手动输入

#### 方式二：手动输入

1. 在 Mac 端查看显示的 IP 地址（例如 `192.168.1.100`）
2. 在 Android 端的 IP 输入框中手动输入该地址
3. 确保格式正确（不要加 `http://` 前缀）
...
5. 在电脑上按 `Cmd+V`（Mac）或 `Ctrl+V`（Windows），或开启自动粘贴，文字直接出现
...
- **自动粘贴开关**：是否自动模拟粘贴快捷键到当前应用
- **响应远程回车开关**：是否执行手机发来的回车指令
- **历史记录**：查看所有同步过的文字，点击/双击可重新复制
...
如果下载后双击应用提示**"应用已损坏"**、**"无法验证开发者"**或**"无法打开"**，这是 macOS 的安全机制限制。

**解决方法**：在终端运行以下命令移除隔离属性

```bash
sudo xattr -cr /Applications/VoiceSyncMac.app
```

**说明**：
- `xattr -cr` 命令会递归移除应用的扩展属性（包括隔离标识）
- 需要输入管理员密码
- 执行后重新打开应用即可正常使用
